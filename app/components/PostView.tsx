  ...
  return (
    <div className="flex flex-col">
      <div className="flex gap-3">
        <Link href={`/profile/${comment.profiles?.username || ''}`} prefetch={false}>
          <ListFallbackImage
        ...
        <div className="ml-12 mt-6 flex flex-col gap-6 relative">
          ...
          {comment.replies.map((reply: TASHComment) => (
            <div key={reply.id} className="flex gap-3 relative">
              <Link href={`/profile/${reply.profiles?.username || ''}`} prefetch={false}>
                <ListFallbackImage
  ...
  return (
    <div className="py-2">
      ...
      <div className="flex items-center mb-1 px-5 gap-4">
        <Link href={`/work/${data.works?.slug || data.works?.id}`} prefetch={false} className="flex items-center gap-4 w-full">
